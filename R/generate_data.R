...
#' )
generate_background_pop <- function(
  years_to_generate = 2015:2024,
  pnrs_per_year = 1000,
  turnover_per_year = 20,
  seed = 2109
  ...
  total_background_rows <- pnrs_per_year +
    (length(years_to_generate) * turnover_per_year)
  ...
  tibble::tibble(
    ...
    foed_dag = sample(
      seq(
        as.Date("1950-01-01"),
        as.Date(paste0(min(years_to_generate) - 1, "-12-31")),
        by = "day"
      ),
      total_background_rows,
      replace = TRUE
...
#'   years_to_generate = 2015:2024
#' )
generate_bef <- function(
  background_df,
  n_samples = 1000,
  years_to_generate = 2015:2024
) {
  # A HELPER FUNCTION FOR YEARLY SAMPLING
  generate_year_sample <- function(
    curr_year,
    background_df,
    n_samples = n_samples
  ) {
  ...
  # EXECUTE MAP & RETURN
  result <- purrr::map(years_to_generate, \(yr) {
    generate_year_sample(yr, background_df, n_samples)
  }) |>
    purrr::list_rbind()
  ...
  generate_lpr_adm_year <- function(curr_year) {
    ...
    dplyr::tibble(
      ...
      recnum = paste0(
        sprintf(
          "%08d",
          sample(0:49999999, n_samples_per_year, replace = FALSE)
        ),
        replicate(
          n_samples_per_year,
      ...
      c_spec = sprintf(
        "%02d",
        sample(0:99, n_samples_per_year, replace = TRUE)
      ),
      year = year_char
    )
  }
...
#' }
#' @export
generate_lpr_a_kontakt <- function(
  background_df,
  n_samples_per_year = 1000,
  ...
  dplyr::tibble(
    ...
    kont_starttidspunkt = t_start +
      runif(total_n, min = 0, max = total_duration_secs),
...
generate_icd_10_diagnoses <- function(
  record_vector,
